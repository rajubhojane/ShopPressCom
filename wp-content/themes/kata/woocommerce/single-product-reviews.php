	...
	<div class="kata-review-summery">
		...
		<div class="overall-rating-wrap">
			...
			<div class="reviews-count">
				<?php
					echo sprintf( wp_kses_post( __( '%s review', 'kata' ), __( '%s reviews', 'kata' ), $count ), wp_kses_post( number_format_i18n( $count ) ) );
				?>
			</div>
		...
		<div id="review_form">
			<?php
				...
				foreach ( $fields as $key => $field ) {
					...
					$field_html .= '<input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" type="' . esc_attr( $field['type'] ) . '" autocomplete="' . esc_attr( $field['autocomplete'] ) . '" value="' . esc_attr( $field['value'] ) . '" placeholder="' . esc_attr( $field['placeholder']) . ($field['required'] ? '*' : '' ) . '" size="30" ' . ( $field['required'] ? 'required' : '' ) . ' /></p>';
				...
				if ( wc_review_ratings_enabled() ) {
					$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating" id="comment-form-rating-label">' . esc_html__( 'Your rating', 'kata' ) . ( wc_review_ratings_required() ? '&nbsp;<span class="required">*</span>' : '' ) . '</label><select name="rating" id="rating" required>
				...
				$comment_form['comment_field'] .= '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="8" placeholder="' . __( 'Your review *', 'kata' ) . '" required></textarea></p>';
